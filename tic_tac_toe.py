...
class State:
    ...
    def is_end(self):
        ...
        reverse_trace = np.trace(np.fliplr(self.data))
...
class Player:
    ...
    def set_symbol(self, symbol):
        ...
        for hash_val in all_states:
            ...
            if is_end:
                self.estimations[hash_val] = 1.0 if state.winner == self.symbol else 0.5 if state.winner == 0 else 0
            else:
    ...
    def act(self):
        state = self.states[-1]
        next_positions = [(i, j) for i in range(BOARD_ROWS) for j in range(BOARD_COLS) if state.data[i, j] == 0]
        ...
        values = sorted([(self.estimations[hash_val], pos) for hash_val, pos in zip(next_states, next_positions)], key=lambda x: x[0], reverse=True)
